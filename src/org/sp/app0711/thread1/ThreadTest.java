...
2)	쓰레드의 생명주기(LifeCycle)
  ...
  -jvm 내부 알고리즘에 의해 랜덤하게 특정 Thread가 선택되고, 이 선택된
   쓰레드의 run() 메서드를 실행하는 단계를 Running이라 한다
...
public class ThreadTest extends JFrame{
	...
	public ThreadTest() {
		...
		//이벤트 연결 코드는 재사용성이 없기 때문에, 되도록이면 코드가 간소화될수록 좋다..
	...
	//메인실행부(=메인쓰레드)는 절대로, 대기상태나 무한루프에 빠트리면 안됨..
	//왜?? 메인실행부(=메인쓰레드)는 1)gui에서 사용자의 이벤트처리, 그래픽처리
	...
	public static void main(String[] args) {
		...
		//메시지가 출력된다..결론) 자바의 실행부라 불렸던 그 녀석은 사실
		//메인 쓰레드였다..메인쓰레드는 프로그램을 운영하는 실행부이므로,
		//반드시 대기상태나, 루프에 빠뜨리지 말자!!(프로그램 먹통 됨)
		/*
		int[] arr= new int[2];
		arr[0]=5;
		arr[1]=7;

...
public class GamePanel extends JPanel implements KeyListener{
	...
	//메인쓰레드를 루프에 빠트리지 않고, 별도의 개발자정의 쓰레드를 만들어 루프를 실행하기 위함...
	//메인쓰레드를 루프나 대기 상태에 빠트리지 않아야 하는 이유?
	//메인쓰레드는 프로그램의 실행과 운영을 담당하는 메인실행부이므로, 대기나 루프에 빠지는 순간
			...
			public void run() {
				while(true) {
					try {
						...
						e.printStackTrace();
					} //1000분의 1초 동안 non-runnable로 진입
											//하지만 지정한 시간이 지나면 스스로 다시 runnable로 복귀
					loop();
				}
			}
		};
	...
	public void loop() {
		...
		repaint();
		//System.out.println("루프수행 중"); //(while문 지움)
	}
}
